...
const GridMotion = ({ items = [], gradientColor = 'black' }) => {
  ...
  useEffect(() => {
    const maxMoveAmount = 300;
    const baseDuration = 10; // Slower duration for continuous motion
    const inertiaFactors = [0.6, 0.4, 0.3, 0.2]; // Different inertia for each row, outer rows slower

    rowRefs.current.forEach((row, index) => {
      if (row) {
        const direction = index % 2 === 0 ? 1 : -1; // Alternate direction of movement for odd/even rows
        gsap.to(row, {
          x: direction * maxMoveAmount, // Move left/right based on row
          duration: baseDuration + inertiaFactors[index % inertiaFactors.length],
          repeat: -1, // Repeat infinitely
          yoyo: true, // Reverse direction at the end of each cycle
          ease: 'linear',
        });
      }
    });
